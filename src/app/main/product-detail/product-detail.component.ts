...
export class ProductDetailComponent implements OnInit {
    ...
    this.userService.Apicommand('/productdetails',this.data)
      .subscribe(
          result => {  
            ...
            this.size_value = this.sizeOfProduct[0]['size_value'];
            this.color_name = this.colorOfProduct[0]['color_name'];
            //console.log(this.results);
            if(this.results['meta_title'] != '' && this.results['meta_title'] !='null'){
              this.product_meta_title = this.results['meta_title'];
            }else{
              this.product_meta_title = this.results['product_name'];
            }
            if(this.results['meta_keyword'] != '' && this.results['meta_keyword'] !='null'){
              this.product_meta_keyword = this.results['meta_keyword'];
            }else{
              this.product_meta_keyword = "";
            }
            if(this.results['meta_description'] != '' && this.results['meta_description'] !='null'){
              this.product_meta_description = this.results['meta_description'];
            }else{
              this.product_meta_description = "";
            }
            this.titleService.setTitle(this.product_meta_title);
            this.meta.updateTag({name : 'keywords', content: this.product_meta_keyword});
            this.meta.updateTag({name : 'description', content: this.product_meta_description});
      ...
      if(this.pincode !='' && this.pincode != null && this.newpincode){
        if((this.sizeOfProduct[0]['fk_size_id'] != '' && this.sizeOfProduct[0]['fk_size_id'] != null) && (this.colorOfProduct[0]['fk_color_id'] != '' && this.colorOfProduct[0]['fk_color_id'] != null) ){
            if((this.size!='' && this.size != null) && (this.color != '' && this.color != null) ){
              ...
              this.toastr.error('Please choose color');
              ...
              this.toastr.error('Please choose size and color');
            }
        }
        ...
        this.toastr.error('Please check your pincode for availablity');
      }
    }else{
      //this.amount_err = 'Amount should be less than 10,00000';
      this.toastr.error('Amount should be less than 10,00000');
    }  
  }
  // Cart List
  buynow(pid,qty){
  ...
  onSubmit() {
    ...
    if(this.pincode=="")
    { 
      this.toastr.error('Please enter your pincode'); 
    ...
    .subscribe(
        data =>{  
          ...
          if(info.available==true){
            ...
            this.toastr.success('Delivery to this pincode is available'); 
          }else{
            //this.pincode_err = 'Delivery to this pincode is not available';
            this.toastr.error('Delivery to this pincode is not available'); 
          }
        },
      error => {
        var info=JSON.parse(JSON.stringify(error));
