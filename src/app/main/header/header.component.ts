...
export class HeaderComponent implements OnInit {
  ...
  request: any;
  otpemail = '';
  cartcount: any;getcartcount:any;
  ...
  ngOnInit() {
    ...
    else if (this.sitetypeval == 'false')
    {
      this.sitetype = false;
    }
    else
    ...
    this.getcartcount = localStorage.getItem('cartcount');
    if(this.getcartcount == 'null' && this.getcartcount == 'undefined') 
    {
      this.cartcount = 0;
    }else{
  ...
  search_key() {
    ...
    if (this.searchkeyword != '') {
      window.location.href = '/product/' + queryParam;
    }
  }
}
